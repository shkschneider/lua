...
print("--[[Array]]--")
_print("type(Array)", type(Array))
local array = Array:new(1, 2, 3, 4, 5, 6, 7, 8, 9)
...
print("--[[Log]]--")
_print("Log.FILENAME", Log.FILENAME)
_print("Log.FILE", Log.FILE)
_print("Log.verbose(...)", Log.verbose)
_print("Log.debug(...)", Log.debug)
_print("Log.info(...)", Log.info)
_print("Log.warning(...)", Log.warning)
_print("Log.error(...)", Log.error)
_print("Log.wtf(...)", Log.wtf)
_print("Log.close()", Log.close)

print("--[[Path]]--")
_print("Path.root", Path.root)
_print("Path.dirname(Path.root)", Path.dirname(Path.root))
_print("Path.basename(Path.root)", Path.basename(Path.root))
_print("Path.split(Path.root)", Path.split(Path.root))

print("--[[Utils]]--")
_print("tostring({...})", tostring({"a", "b", "c"}))
_print("rgb(255,255,255)", rgb(255, 255, 255))
_print("rgba(255,255,255,0)", rgba(255, 255, 255, 0))
_print("loop(2,f(i))", "")
loop(2, function (i) _print("", i) end)
_print("lines('hello\\nworld')", lines("hello\nworld"))
_print("lines('hello world', ' ')", lines("hello\nworld", " "))
_print("math.randomseed", math.randomseed)
_print("randomboolean()", randomboolean())
_print("randomtable({1,2,3})", randomtable({1, 2, 3}))
_print("math.clamp(10,1,9)", math.clamp(10, 1, 9))
_print("math.sign(10)", math.sign(-10))

print("--[[Version]]--")
local oldVersion = Version(1,2,3)
...
local newVersion = Version(3,2,1)
